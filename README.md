...
│
├── SQL/
│   └── SQLQueries.sql          # Complete data cleaning SQL script
│
├── Dataset/
...
- **Data Type Conversion**: CONVERT() function for date standardization
- **String Manipulation**: SUBSTRING(), CHARINDEX(), PARSENAME() for address parsing
- **Self-Joins**: Joining tables to themselves for data imputation
- **Window Functions**: ROW_NUMBER() for duplicate detection
- **Common Table Expressions (CTEs)**: For complex query organization
- **CASE Statements**: For conditional value transformations
- **ISNULL()**: For handling null values
...
- Problem-solving with complex data issues
- Attention to detail in data transformation
...
- Automated testing for data quality rules

---

*This project showcases practical data cleaning skills essential for data analysis, business intelligence, and data engineering roles.*
